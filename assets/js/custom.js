...
(function () {
    ...
    $(window).on("scroll", function () {
      ...
      var margin = Math.max(
        jQuery(window).scrollTop() +
          $(".st_mega_menu_container").outerHeight() -
          $obj.offset().top,
        0
      );
      margin = Math.min(
  ...
  _self.home_tabs = function () {
        ...
        mutations.forEach(function (mutation) {
          if (mutation.attributeName === "class") {
            ...
            if (attributeValue.search(/active/) != -1) {
              ...
              $header
                .parents(".pc_slider_block_container")
                .first()
                .attr(
                  "data-tab",
                  $(mutation.target).attr("href").replace("#", "")
                );
            }
          }
        });
    ...
    _self.facets_classes = function () {
      ...
      jQuery("#search_filters .facet").each(function (i, v) {
        if (
          jQuery(v).find(
            "input[checked], option[selected], .checkbox-inline.active"
          ).length
        ) {
          ...
          if (
            parseFloat($range.data("min")) !=
              parseFloat($range.data("lower")) ||
            parseFloat($range.data("max")) != parseFloat($range.data("upper"))
          ) {
            jQuery(v).addClass("active");
            has_something = true;
          }
      ...
      if (
        has_something ||
        jQuery("#main .js-search-filters-clear-all").length
      ) {
    ...
    _self.product_updated = function () {
      $(".product-information .product-reference").toggle(
        $.trim(
          $(
            ".product-information .product-reference .pro_extra_info_content"
          ).text()
        ) != ""
      );
      $(".prod_weight").toggle(
        parseInt($.trim($(".prod_weight span").text())) > 0
      );
    };
  ...
  _self.product_tabs_column_sticky = function () {
    ...
    if (scroll > 0 && scroll + header > top && $div.is(":visible")) {
      ...
      $div.find(".nav-link, .short_list .btn-line").each(function () {
        ...
        if (
          $section.offset().top < scroll + header + $div.height() &&
          $section.offset().top + $section.height() >
            scroll + header + $div.height()
        ) {
  ...
  _self.product_left_content_sticky = function () {
    ...
    let top = Math.min(
      Math.max(scroll - column.offset().top, 0),
      column.height() - div.height()
    );
    ...
    if (
      top > 0 &&
      $(".product_left_column").offset().top == $(".box-right").offset().top
    ) {
  ...
  _self.filter_count_set = function (qty) {
    ...
    dom.html(
      qty +
        ...
        _self.odmien(qty, dom.data("one"), dom.data("two"), dom.data("five")) +
        "</span>"
    );
  };
  ...
  _self.odmien = function (qty, one, two, five) {
    ...
    if (
      jQuery.inArray(qty % 10, [2, 3, 4]) != -1 &&
      jQuery.inArray(qty % 10, [12, 13, 14]) == -1
    )
      return two;
    return five;
  };
  ...
  $(document).on(
    ...
    function () {
      $(this)
        .closest(".form-group")
        .toggleClass("st-has-error", !_self.verify($(this)));
      $(this)
        .closest(".form-group")
        .toggleClass("st-not-empty", $(this).val() != "");
      $(this)
        .closest(".form-group")
        .toggleClass("st-is-empty", $(this).val() == "");
    }
  );
  ...
  jQuery(document).ready(function ($) {
    ...
    prestashop.on("stUpdateCart", function (t) {
      if (typeof t.resp != "undefined") {
        jQuery.each(t.resp.cart.products, function (i, v) {
          jQuery('[data-id-product="' + v.id_product + '"] [data-q]')
            .attr("data-q", Math.max(v.stock_quantity - v.quantity))
            .find("em")
            .text(Math.max(v.stock_quantity - v.quantity));
        });
      }

      if (t.reason.linkAction == "delete-from-cart") {
        var $q = jQuery(
          '[data-id-product="' + t.reason.idProduct + '"] [data-q]'
        );
    ...
    prestashop.on("updateProductList", function (t) {
      if (t.pagination.current_page == 1) jQuery(".one_page_desc").show();
      else jQuery(".one_page_desc").hide();
      ...
      if (t.seo != "undefined") {
        ...
        $('meta[name="robots"]').attr(
          "content",
          (t.seo.index ? "index" : "noindex") + ",follow"
        );
    ...
    if (jQuery(".product_count[data-total]").length) {
      _self.filter_count_set(
        jQuery(".product_count[data-total]").data("total")
      );
    } else {
    ...
    var observer = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        if (mutation.attributeName === "class") {
          ...
          $(".menu_blur.color-bg").toggleClass(
            "blur_effect",
            attributeValue.search("open") != -1
          );
          $(".menu_blur").toggleClass(
            "blur_effect",
            attributeValue.search("open") != -1
          );
          $("#st-container").toggleClass(
            "mobile_search",
            attributeValue.search("sidebar_opened") != -1
          );
        }
    ...
    jQuery(document).on("click", ".open-sidebar", function () {
      jQuery("#st-container").toggleClass(
        "sidebar_plus",
    ...
    observer = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        if (mutation.attributeName === "class") {
          var attributeValue = $(mutation.target).prop(mutation.attributeName);
          if (attributeValue.search("open_bar_right") == -1)
            jQuery("#st-container").removeClass("sidebar_plus");
        }
      });
    });
  ...
  $(document).on(
    "click",
    '[data-link-action="remove-from-cart"], [data-link-action="delete-from-cart"]',
    function (e) {
      e.preventDefault();

      let id = jQuery(this)
        .attr("href")
        .match(/id_product=([0-9]+)/)[1];
      let name = jQuery(this)
        .parents(".flex_container")
        .first()
        .find(".product-name")
        .text();
      let price = jQuery(this)
        .parents(".small_cart_info")
        .first()
        .find(".mini-cart-price-gt, .mini_price")
        .text()
        .replace(/\s/g, "")
        .match(/[0-9,]+/)[0]
        .replace(",", ".");
...
function custom_swiper_options_init() {
  function t(t) {
    (void 0 === swiper_options[t].autoplayDisableOnInteraction ||
      swiper_options[t].autoplayDisableOnInteraction) &&
      e(t);
  }
  ...
  function e(t) {
    clearTimeout(r[t]),
      (s[t] = !0),
      $(swiper_options[t].id_st + " .swiper_custom_bar").hide();
  }
  ...
  function i(t) {
    ...
    (a[t] = 0),
      ...
      (r[t] = setInterval(function () {
        !1 === o[t]
          ? ((a[t] += 1e3 / swiper_options[t].bar_time),
            $(swiper_options[t].id_st + " .swiper_custom_bar").css({
              width: a[t] + "%",
            }),
            a[t] >= 100 &&
              (clearTimeout(r[t]),
              (void 0 !== swiper_options[t].autoplay &&
                swiper_options[t].autoplay) ||
                n[t].slideNext()))
          : ((a[t] = 0),
            (void 0 === swiper_options[t].autoplayDisableOnInteraction ||
              swiper_options[t].autoplayDisableOnInteraction) &&
  ...
  "undefined" != typeof swiper_options &&
    ...
    $.each(swiper_options, function (a, r) {
      if (void 0 === r.st_inited) {
        var s = $.extend({}, r);
        void 0 !== s.st_full_screen &&
          s.st_full_screen &&
          $(window).height() > s.st_height &&
          $(r.id_st).height($(window).height()),
          (n[a] = new Swiper(r.id_st, s)),
          (swiper_options[a].st_inited = !0),
          void 0 !== s.custom_progress_bar &&
            (i(a),
            n[a].on("slideChangeStart", function (t) {
              i(a);
            }),
  ...
  if (
    typeof stowlcarousel_array !== "undefined" &&
    stowlcarousel_array.length
  ) {
...
function initMobileSubMenu() {
  ...
  sidemenuButtons.forEach((btn) => {
    ...
    const sidemenuCloseButton = sidemenu.querySelector(
...
function initDesktopSubmenuToggle() {
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      document
        .querySelectorAll("[data-js-tab-btn]")
        .forEach((b) => b.classList.remove("active"));
      document
        .querySelectorAll("[data-js-tab]")
