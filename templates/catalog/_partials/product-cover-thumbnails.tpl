...
{else}
    ...
    {/if}
{/if}
<div class="images-container pro_number_{if $slidesPerView<$image_count}{$slidesPerView}{else}{$image_count}{/if}">
...
<div class="pro_gallery_top_container {if $sttheme.product_thumbnails==1 || $sttheme.product_thumbnails==2} col-{$gallery_top_width_v_mobile|replace:'.':'-'} col-md-{$gallery_top_width_v|replace:'.':'-'} {/if}{if $sttheme.product_thumbnails==1} push-{$gallery_thumbnails_width_v_mobile|replace:'.':'-'} push-md-{$sttheme.gallery_thumbnails_width_v|replace:'.':'-'} {/if} mb-3">
  ...
  {block name='product_cover'}
    ...
    {*{foreach $pro_popup_trigger as $index => $image}
        <a href="{$image.bySize.superlarge_default.url}" class="pro_popup_trigger st_popup_image st_pro_popup_image replace-2x layer_icon_wrap" data-group="pro_gallery_popup_trigger" title="{if $image.legend}{$image.legend}{else}{$product.name}{/if}"><i class="fto-resize-full"></i></a>
      {/foreach}*}
      </div>
    {/if}
    ...
    //<![CDATA[
        ...
        swiper_options.push({
            ...
            roundLengths: true,
            lazyLoading: {if $sttheme.lazyload_main_gallery}false{else}true{/if},
...
<div class="pro_gallery_thumbs_container {if $sttheme.product_thumbnails==1 || $sttheme.product_thumbnails==2} col-{$gallery_thumbnails_width_v_mobile|replace:'.':'-'} col-md-{$sttheme.gallery_thumbnails_width_v|replace:'.':'-'} pro_gallery_thumbs_vertical {elseif $sttheme.product_thumbnails==3} pro_gallery_thumbs_grid {else} pro_gallery_thumbs_horizontal {/if}{if $sttheme.product_thumbnails==1} pull-{$gallery_top_width_v_mobile|replace:'.':'-'}  pull-md-{$gallery_top_width_v|replace:'.':'-'} {/if}">
  {block name='product_images'}
    <div class="swiper-container pro_gallery_thumbs {if $sttheme.slider_thumbnails_hook}button_nohover{/if} swiper-button-lr {if $sttheme.thumbs_direction_nav==3} swiper-navigation-rectangle {elseif $sttheme.thumbs_direction_nav==2} swiper-navigation-arrow {else} swiper-navigation-circle {/if} {if $sttheme.product_thumbnails==0} swiper-small-button {/if} {if $sttheme.product_gallerys} hightlight_curr_thumbs {/if}" {if $language.is_rtl} dir="rtl" {/if}>
        <div class="swiper-wrapper">
            {assign var="curr_combination_thumbs" value=[]}
            {foreach $product.images as $index => $image}
              {$curr_combination_thumbs[]=$image.id_image}
       ...
       {if $sttheme.slider_thumbnails_hook}
    ...
    {if $sttheme.product_thumbnails!=3}
        ...
        swiper_options.push({
            ...
            spaceBetween: {(int)$sttheme.slider_thumbnails_zone},
            slidesPerView: 'auto',
            {if $sttheme.product_thumbnails==1 || $sttheme.product_thumbnails==2}
            direction: 'vertical',
            ...
            roundLengths: true,
            lazyLoading: {if $pro_gallery_initial || $sttheme.lazyload_main_gallery}false{else}true{/if},
            lazyLoadingInPrevNext: true,
            lazyLoadingInPrevNextAmount: 2,
            initialSlide: {$pro_gallery_initial}
        {literal}
