    ...
    {foreach $stickers as $ststicker}
      {if !in_array($ststicker.id_st_sticker, explode(',', str_replace(' ', '', $p_c.id_strickers)))}
        {if in_array($ststicker.sticker_position, $sticker_position)}
          {if $ststicker.frame_info == 1 && $ststicker.border_width_iframe > 0}<div
              class="sticker_border_{$ststicker.id_st_sticker} sticker-border"></div>
          {elseif isset($ststicker.image_multi_lang) && $ststicker.image_multi_lang}
              <img src="{$ststicker.image_multi_lang}" class="sticker-img" style="width: {$ststicker.text_width}px;height: {$ststicker.text_height}px;">
          {/if}
        {/if}
      {/if}
    {/foreach}
      ...
      {foreach $ststickers as $ststicker}
        {if !in_array($ststicker.id_st_sticker, explode(',', str_replace(' ', '', $p_c.id_strickers)))}
          {if in_array($ststicker.sticker_position, $sticker_position) && ( ($flag.type=='new' &&  $ststicker.type==1) || ($flag.type=='on-sale' &&  $ststicker.type==2) || ($flag.type=='online-only' &&  $ststicker.type==5) || ($flag.type=='pack' &&  $ststicker.type==6) )}
            {if $ststicker.frame_info == 1 && $ststicker.border_width_iframe > 0}<div
              class="sticker_border_{$ststicker.id_st_sticker} sticker-border"></div>{/if}
          {/if}
        {/if}
      {/foreach}
    {/foreach}
