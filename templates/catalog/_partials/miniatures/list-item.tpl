 ...
 *}
  ...
  <div class="products product_list {if $sttheme.infinite_scroll==1} waypoint {/if}{if $for_w=='category' && $sttheme.list_grid} list {else} row grid {/if}" data-classnames="{$smarty.capture.product_gide_class}">
    ...
    {if $curr_iteration%$pro_per_xs == 0} last-item-of-portrait-line{elseif $curr_iteration%$pro_per_xs == 1} first-item-of-portrait-line{/if}{if $curr_iteration > ($nbLi - $totModuloPortrait)} last-portrait-line{/if}{if $curr_index < $pro_per_xs} first-portrait-line{/if}" style="order: {$curr_iteration};">
    ...
    {if $sttheme.big_next && $for_w=="category" && !$sttheme.infinite_scroll && count($listing.pagination.pages)>3 && $listing.pagination.items_shown_to<$listing.pagination.total_items}
      ...
      {if $nbLiNext%$pro_per_xs == 0} last-item-of-portrait-line{elseif $nbLiNext%$pro_per_xs == 1} first-item-of-portrait-line{/if}{if $nbLiNext > ($nbLi - $totModuloPortrait)} last-portrait-line{/if}{if $nbLi < $pro_per_xs} first-portrait-line{/if} big_page_next_wrap" style="order: 100000">
