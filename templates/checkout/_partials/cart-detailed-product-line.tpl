...
<div class="product-line-grid container-fluid">
  ...
  <div class="product-line-grid-left col-md-2 col-3">
    <span class="product-image media-middle">
      <a href="{$product.url}"><img src="{if isset($product.cover.bySize.cart_default.url) && $product.cover.bySize.cart_default.url}{$product.cover.bySize.cart_default.url}{elseif isset($urls.no_picture_image)}{$urls.no_picture_image.bySize.cart_default.url}{else}{$sttheme.img_prod_url}{$sttheme.lang_iso_code}-default-cart_default.jpg{/if}" width="{$product.cover.bySize.cart_default.width}" height="{$product.cover.bySize.cart_default.height}" alt="{$product.name|escape:'quotes'}" /></a>
    </span>
  </div>
  ...
  <div class="product-line-grid-body col-md-5 col-7">
    ...
    <div class="product-line-info product-price {if $product.has_discount}has-discount{/if} mar_b6">
      ...
      {assign var='price_tax_cart_products' value=Configuration::get('STSN_PRICE_TAX_CART_PRODUCTS')}
        <span class="price">{$product.price} {if $price_tax_cart_products > 0}<span class="name_price">{$product.labels.tax_long}</span>{/if}</span>
    ...
    <div class="product-line-info cart_manufacturer">
    {$product.manufacturer_name|escape:'htmlall':'UTF-8'}</div>{/if}
    
    {assign var='ref_cart_products' value=Configuration::get('STSN_REF_CART_PRODUCTS')}
    <div class="product-line-info ref">
    {if $ref_cart_products > 0 && $product.reference_to_display}<span class="label">{l s='Reference' d='Shop.Theme.Transformer'}:</span> <span class="value">{$product.reference_to_display}</span>{/if}
    </div>
          ...
          <div class="col-md-6 col-2">
            <span class="product-price price">
              <strong>{*can not remove this strong tag*}
                ...
                {else}
                  {$product.total}
                {/if} {if $price_tax_cart_products > 0}<span class="name_price">{$product.labels.tax_long}</span>{/if}
              </strong>
            </span>
          </div>
        </div>
