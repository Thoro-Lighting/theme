          ...
          //improve this
            ...
            else if(resp.success==1)
            {
              if(resp.sidebar && $('#side_loved').length){
                let classes = $('#side_loved').attr('class');
                $('#side_loved').replaceWith(resp.sidebar);
                $('#side_loved').addClass(classes)
              }
...
function stLovedProductRemove(id_source, type)
{
	...
	$.ajax({
      ...
      $('.loved_remove_product.active').removeClass('active');
			if (data.success==1) {
        let item = $('.loved_product_item[data-id_source='+id_source+']');
        if ( item.parent('.loved_product_item').length ) item.parent('.loved_product_item').remove()
        else item.remove();

        $('.watched_tabs .nav-link.active span').text('(' + ( parseInt($('.watched_tabs .nav-link.active span').text().substring(1)) - 1 ) + ')')
			}
