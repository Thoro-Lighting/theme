...
*}
  ...
  <div class="st_posts {if $category_layouts== 6}list_masonry_cat{/if} product_list row grid  {if $category_layouts == 7}d-block{/if} category_list">
  
    {foreach $blogs as $index => $blog name="data"}
    ...
    {if $category_layouts == 7}    
    <div class="product_list_item cat_blog_art
    {if $smarty.foreach.data.iteration == 1}float-left col-lg-5 big{/if}
    {if $smarty.foreach.data.iteration == 2}float-left col-lg-4 half{/if}
    {if $smarty.foreach.data.iteration == 3}float-left col-lg-3 half{/if}
    {if $smarty.foreach.data.iteration == 4}float-left col-lg-7 half{/if}
    {if $smarty.foreach.data.iteration == 5}float-left col-lg-6 half{/if}
    {if $smarty.foreach.data.iteration == 6}float-left col-lg-6 half{/if}
    {if $smarty.foreach.data.iteration == 7}float-left col-lg-8 half{/if}
    {if $smarty.foreach.data.iteration == 8}float-left col-lg-4 half{/if}
    {if $smarty.foreach.data.iteration == 9}float-left col-lg-6 half{/if}
    {if $smarty.foreach.data.iteration == 10}float-left col-lg-6 half{/if}
    {if $smarty.foreach.data.iteration == 11}float-left col-lg-4 half{/if}
    {if $smarty.foreach.data.iteration == 12}float-left col-lg-4 half{/if}
    {if $smarty.foreach.data.iteration == 13}float-left col-lg-4 half{/if}
    {if $smarty.foreach.data.iteration == 14}float-left col-lg-6 half{/if}
    {if $smarty.foreach.data.iteration == 15}float-left col-lg-6 half{/if}
    {if $smarty.foreach.data.iteration == 16}float-left col-lg-8 half{/if}
    {if $smarty.foreach.data.iteration == 17}float-left col-lg-4 half{/if}
    {if $smarty.foreach.data.iteration == 18}float-left col-lg-6 half{/if}
    {if $smarty.foreach.data.iteration == 19}float-left col-lg-6 half{/if}">
   {include file='module:stblog/views/templates/slider/post-mosaic.tpl'}
    </div>
    
  {else}
    
      <div class="product_list_item cat_blog_art {if $is_list} clearfix {else} col-fw-{(12/$pro_per_fw)|replace:'.':'-'} col-xxl-{(12/$pro_per_xxl)|replace:'.':'-'} col-xl-{(12/$pro_per_xl)|replace:'.':'-'} col-lg-{(12/$pro_per_lg)|replace:'.':'-'} col-md-{(12/$pro_per_md)|replace:'.':'-'} col-sm-{(12/$pro_per_sm)|replace:'.':'-'} col-{(12/$pro_per_xs)|replace:'.':'-'} {/if}
   ...
   {if $category_layouts == 6}
   <script src="https://unpkg.com/masonry-layout@4.2.2/dist/masonry.pkgd.min.js"></script>
   <script>
            window.addEventListener('load', function(event) {
                  new Masonry('.list_masonry_cat', {
