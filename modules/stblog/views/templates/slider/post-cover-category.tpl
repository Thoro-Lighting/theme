...
{assign var='blog_image_type' value=Configuration::get('ST_BLOG_BLOG_IMAGE_TYPE')}


     {if ($stblog.display_image != 0) && ((isset($blog.cover) && $blog.cover) || $blog.cover_2.links.large.image) }
          {if $blog.type==1}
              {if $is_lazy}<i class="swiper-lazy-preloader fto-spin5 animate-spin"></i>{/if}
              
              {if $stblog.text_image_bg_all}<div title="{$blog.name}" class="pro_second_box_img"></div>{/if}
              
              <div class="blog_image" itemprop="image" itemscope itemtype="https://schema.org/ImageObject">

                {if (!empty($blog.cover_2.links.large.image) && ($stblog.display_image == 3 || ($blog.img_on_category == 2 && $stblog.display_image == 1)))}
                  <a href="{$blog.link}" rel="bookmark" title="{$blog.name}">
                      {if $stblog.text_image_bg_all}<div class="pro_second_box_img"></div>{/if}
                    <img {if $is_lazy}data-src{else}src{/if}="{$blog.cover_2.links.large.image}" alt="{$blog.name}" width="{$blog.cover_2.links.large.width}" height="{$blog.cover_2.links.large.height}" class="{if $is_lazy} swiper-lazy {/if} front-image" />
                  </a>
                  <meta itemprop="url" content="{$blog.cover_2.links.large.image}">
                  <meta itemprop="width" content="{$blog.cover_2.links.large.width}">
                  <meta itemprop="height" content="{$blog.cover_2.links.large.height}">
                {else}
                  <a href="{$blog.link}" rel="bookmark" title="{$blog.name}">
                      {if $stblog.text_image_bg_all}<div class="pro_second_box_img"></div>{/if}
