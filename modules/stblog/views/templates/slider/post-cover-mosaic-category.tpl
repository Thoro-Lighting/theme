...
{assign var='blog_image_type' value=Configuration::get('ST_BLOG_BLOG_IMAGE_TYPE')}
     {if ($stblog.display_image != 0) && ((isset($blog.cover) && $blog.cover) || $blog.cover_2.links.large.image) }
          {if $blog.type==1}
              ...
              <div class="blog_image" itemprop="image" itemscope itemtype="https://schema.org/ImageObject"  style="background-image: url({if (!empty($blog.cover_2.links.large.image) && ($stblog.display_image == 3 || ($blog.img_on_category == 2 && $stblog.display_image == 1)))}{$blog.cover_2.links.large.image}{else}{$blog.cover.links.{$blog_image_type}.image}{/if})">
