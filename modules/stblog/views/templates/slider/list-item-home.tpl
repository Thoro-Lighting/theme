...
*}
  ...
  <div class="st_posts {if $display_as_grid== 6}list_masonry_{$p_c.id_st_blog_slider}{/if} product_list {if $is_list} list {else} row grid {/if} {if $p_c.grid == 7}d-block{/if}">
   ...
   {include file='module:stblog/views/templates/slider/post-home-mosaic.tpl'}
    </div>
  ...
  {else}
    ...
    {if $curr_iteration%$pro_per_xs == 0} last-item-of-portrait-line{elseif $curr_iteration%$pro_per_xs == 1} first-item-of-portrait-line{/if}{if $curr_iteration > ($nbLi - $totModuloPortrait)} last-portrait-line{/if}{if $curr_index < $pro_per_xs} first-portrait-line{/if}">
        {include file='module:stblog/views/templates/slider/post-home.tpl'}
         </div> 
        {/if}
    {/foreach}
  </div>