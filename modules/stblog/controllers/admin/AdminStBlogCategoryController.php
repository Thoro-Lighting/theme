...
class AdminStBlogCategoryController extends ModuleAdminController
{
	...
	public function postProcess()
	{
		...
		elseif (Tools::getValue('submitDel'.$this->table) || Tools::isSubmit('submitBulkdelete'.$this->table))
		{
			if ($this->tabAccess['delete'] === '1')
			{
				...
				$this->errors[] = Tools::displayError('You do not have permission to delete this.');
		} elseif ( Tools::isSubmit('delete_image_cat') ) {

			$file = _PS_UPLOAD_DIR_.'stblogc/' . $this->object->id . '.jpg';
            if ( file_exists($file) ) unlink($file);

            Tools::redirectAdmin(self::$currentIndex.'&update'.$this->table.'&id_st_blog_category='.(int)$this->object->id.'&token='.Tools::getValue('token'));
		}
 		parent::postProcess();
	}
	...
	public function renderForm()
	{
        ...
        if ($this->object->id) {
        	$file = _PS_UPLOAD_DIR_.'stblogc/' . $this->object->id . '.jpg';
            if ( file_exists($file) )
                $image_cat = '<img style="max-width: 200px" src="/upload/stblogc/' . $this->object->id . '.jpg" /><p><a class="btn btn-default st_blog_cover_image" href="' . self::$currentIndex.'&update'.$this->table.'&id_st_blog_category='.(int)$this->object->id.'&token='.Tools::getValue('token') . '&delete_image_cat"><i class="icon-trash"></i> Delete</a></p>';
        }
		...
		$this->fields_form = array(
			...
			'input' => array(
				...
				array(
					'type' => 'switch',
					'label' => $this->trans('Displayed name category:', array(), 'Admin.Theme.Transformer'),
					'name' => 'visible_title',
					'required' => false,
					'is_bool' => true,
					'values' => array(
				...
				array(
                    'type' => 'file',
                    'label' => $this->trans('Photo of the author of the post:', array(), 'Admin.Theme.Transformer'),
                    'name' => 'image_cat',
                    'image' => $image_cat,
					'desc' => $this->trans('Field used for post authors.', array(), 'Modules.Stblog.Admin').'',
		
                ),
				
				array(
					'type' => 'text',
					'label' => $this->trans('Who is the author:', array(), 'Modules.Stblog.Admin'),
					'name' => 'autor_whois',
					'lang' => true,
					'hint' => $this->trans('Invalid characters:', array(), 'Modules.Stblog.Admin').' <>;=#{}',
				    'desc' => $this->trans('Field used for post authors. Description of who he is, e.g. nutritionist, doctor.', array(), 'Modules.Stblog.Admin').'',
				),
				
								
				array(
					'type' => 'text',
					'label' => $this->trans('Autor title:', array(), 'Modules.Stblog.Admin'),
					'name' => 'autor_title',
					'lang' => true,
					'hint' => $this->trans('Invalid characters:', array(), 'Modules.Stblog.Admin').' <>;=#{}',
				    'desc' => $this->trans('Field used for post authors. Description of the authors title, e.g. mgr, dr.', array(), 'Modules.Stblog.Admin').'',
	...
	public function postCover($obj) {
        if (isset($_FILES['image_cat']['tmp_name']) && !empty($_FILES['image_cat']['tmp_name'])) {
            $dir = _PS_UPLOAD_DIR_.'stblogc/';
            if ( !file_exists($dir) ) mkdir($dir);

            // Check image validity
			$max_size  = (int)Configuration::get('PS_PRODUCT_PICTURE_MAX_SIZE');
			if ($error = ImageManager::validateUpload($_FILES['image_cat'], Tools::getMaxUploadSize($max_size)))
				return $error;
