...
class StBlogImageClass extends ObjectModel
{
    ...
    public static $imageTypeDefault = array(
        '1'  => array(
            ...
            'home'     => 'views/img/home-default.jpg',
            'half'     => 'views/img/half-default.jpg',
        ...
        if (!count(self::$imageTypeDef))
            self::$imageTypeDef = array(
                '1'  => array(
                    ...
                    'home'     => array(
                        (int)Configuration::get('ST_BLOG_IMG_GALLERY_HO_W'), 
                        (int)Configuration::get('ST_BLOG_IMG_GALLERY_HO_H')
                    ),
                    
                    'half'     => array(
                        (int)Configuration::get('ST_BLOG_IMG_GALLERY_HL_W'), 
                        (int)Configuration::get('ST_BLOG_IMG_GALLERY_HL_H')
                    ),
    ...
    public function uploadImage($name)
	{
		if (isset($_FILES[$name]['tmp_name']) && !empty($_FILES[$name]['tmp_name']))
		{
            ...
            if ( !$this->cropImages($tmp_name) ) return false;
    ...
    public static function getImageLinks($image,$type=null)
	{
	    ...
	    foreach($types[$type] as $k=>$v)
        {
            ...
            /*
				2019-09-27
            */
            if ( file_exists(_PS_ROOT_DIR_ . str_replace(__PS_BASE_URI__,'/',$uri_path)) ) {
            	list($v[0], $v[1]) = getimagesize(_PS_ROOT_DIR_ . str_replace(__PS_BASE_URI__,'/',$uri_path));
            }
