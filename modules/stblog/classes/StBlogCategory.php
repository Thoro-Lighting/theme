...
class StBlogCategory extends ObjectModel
{
	...
	/** @var string Object last modification date */
	public $visible_title = 1;
	
	/** @var string autor_title */
	public $autor_title;
	...
	public static $definition = array(
		...
		'fields' => array(
	        ...
	        'visible_title' => 		array('type' => self::TYPE_BOOL, 'validate' => 'isBool', 'required' => true),
	        ...
	        'autor_title' => 		array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isGenericName', 'size' => 128),
	        'autor_whois' => 		array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isGenericName', 'size' => 128),
	...
	public static function getCategories($id_parent = 0, $id_lang, $active = true)
	{
		...
		SELECT c.`id_st_blog_category`,c.`id_parent`,c.`is_root_category`,c.`level_depth`,cl.`name`,cl.`link_rewrite`, cl.`autor_title`, cl.`autor_whois`, cl.`description`, COUNT(DISTINCT cb.`id_st_blog`) AS posts
        ...
        LEFT JOIN `'._DB_PREFIX_.'st_blog_category_blog` cb ON c.`id_st_blog_category` = cb.`id_st_blog_category`
        ...
        if($result)
    		foreach ($result as $row)
            {
                ...
                /*
					iWeb - ID dzieci - 2018-12-19
                */
                $row['child_ids'] = array();
                foreach ($row['child'] as $child) {
                	if ( !empty($child['child_ids']) ) $row['child_ids'] = array_merge($row['child_ids'], $child['child_ids']);
                	$row['child_ids'][] = $child['id_st_blog_category'];
                }

                $row['url'] = Context::getContext()->link->getModuleLink('stblog', 'category', array('id_st_blog_category'=>$row['id_st_blog_category'], 'rewrite'=>$row['link_rewrite']));

                $file = _PS_UPLOAD_DIR_.'stblogc/' . $row['id_st_blog_category'] . '.jpg';
                $row['image'] = file_exists($file) ? '/upload/stblogc/' . $row['id_st_blog_category'] . '.jpg' : '';
	...
	public function getSubCategories($id_lang, $active = true)
	{
		...
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
		SELECT c.*, cl.id_lang, cl.name, cl.description, cl.link_rewrite, cl.meta_title, cl.autor_title, cl.meta_keywords, cl.meta_description
		FROM `'._DB_PREFIX_.'st_blog_category` c
