...
class StBlogClass extends ObjectModel
{
   ...
   /** @var integer type: */
    public $article_blog_view;
    
       /** @var integer type: */
    public $article_blog_autor;
	...
	/** @var integer date */
	public $article_blog_date;
	
	/** @var integer date icon */
	public $article_blog_date_icon;
	
	/** @var integer article views */
	public $article_blog_view_art;
	
	/** @var integer article loved */
	public $article_blog_loved;
	
	/** @var integer article category */
	public $article_blog_category;
	
	   /** @var integer type: */
    public $img_on_category;
	...
	public static $definition = array(
		...
		'fields' => array(
	        ...
	        'article_blog_autor' => 	array('type' => self::TYPE_INT),
	        'img_on_category' => 	array('type' => self::TYPE_INT),
	        'article_blog_top' => 	array('type' => self::TYPE_INT),
            'article_blog_date' => 	array('type' => self::TYPE_INT),
	        'article_blog_date_icon' => 	array('type' => self::TYPE_INT),
	        'article_blog_view_art' => 	array('type' => self::TYPE_INT),
	        'article_blog_loved' => 	array('type' => self::TYPE_INT),  
	        'article_blog_category' => 	array('type' => self::TYPE_INT),    
		...
		if (parent::delete())
        {
            ...
            foreach($images AS $img)
            {
            	/*
					iWeb - 2019-09-27
            	*/
            	$image = new StBlogImageClass('3', $img['id_st_blog_image']);
    ...
    /*
		iWeb - 2019-09-27
    */
    public function getCoverUrl($size = 'thumb', $type = 1)
    {
        if (!$this->id)
            return false;
        ...
        WHERE type = ' . (int)$type . ' AND id_st_blog = '.(int)$this->id.'
        ');
        if ($id_image)
        {
            $img = new StBlogImageClass($type, $id_image);
    ...
    public static function getBlogDetials($id_lang, $row, Context $context = null)
    {
        ...
        if($row['type']==1)
        {
            ...
            $cover = StBlogImageClass::getCoverImage($id_st_blog,$id_lang,3);
            $row['cover_2'] = StBlogImageClass::getImageLinks($cover,3);
        }
        if($row['type']==2)
        {
            $cover = StBlogImageClass::getCoverImage($id_st_blog,$id_lang,1);
        ...
        /*
            iWeb - 2019-08-30
        */
        if ( !class_exists('StLovedProductClass') ) Module::getInstanceByName('stlovedproduct');
        $row['is_loved'] = $context->customer->isLogged() && StLovedProductClass::exists($row['id_st_blog'], $context->customer->id, 2);
