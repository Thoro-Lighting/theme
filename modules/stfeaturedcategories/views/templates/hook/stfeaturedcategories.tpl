...
{assign var='display_text' value=Configuration::get('ST_PRO_CATE_F_C_DISPLAY_TEXT')}
{assign var='display_show_more' value=Configuration::get('ST_PRO_CATE_F_C_DISPLAY_SHOW_MORE')}
{assign var='display_text_hover' value=Configuration::get('ST_PRO_CATE_F_C_DISPLAY_TEXT_HOVER')}
{assign var='position_buttons' value=Configuration::get('ST_PRO_CATE_F_C_POSITION_BUTTONS')}
{assign var='slider_static' value=Configuration::get('ST_PRO_CATE_F_C_SLIDER_STATIC')}
...
{if $title_position !=3} 
    <div class="title_block flex_container title_align_{(int)$title_position} title_style_{(int)$sttheme.heading_style} mb-0">
        <div class="flex_child title_flex_left"></div>
        <div class="title_block_inner">{l s='Featured categories' d='Shop.Theme.Transformer'}</div>
        <div class="flex_child title_flex_right"></div>
        {if $slider_static == 0 && $direction_nav==1 && isset($featured_categories) && $featured_categories}
        ...
        <div class="block_content">
            <div class="swiper-container products_sldier_swiper position_buttons_{$position_buttons} {if $direction_nav>1} swiper-button-lr {if $direction_nav==6 || $direction_nav==7} swiper-navigation-circle {elseif $direction_nav==4 || $direction_nav==5} swiper-navigation-rectangle  {elseif $direction_nav==8 || $direction_nav==9} swiper-navigation-arrow {elseif $direction_nav==2 || $direction_nav==3} swiper-navigation-full {/if} {if $direction_nav==2 || $direction_nav==4 || $direction_nav==6|| $direction_nav==8} swiper-navigation_visible {/if}{/if}" {if $sttheme.is_rtl} dir="rtl" {/if}>
                ...
                <div class="featured_categories_item swiper-slide">
                   ...
                   {if $display_text == 0} <div class="category_img_text">
                    <h3 title="{$category.title}" class="s_title_block">{if $category.title}{$category.title|truncate:35:'...'}{else}{$category.name|truncate:35:'...'}{/if}</h3>
                  <p>{$category.subtitle nofilter}</p>
                  </div>{/if}
                    ...
                    {if $display_text == 1}<h3 class="s_title_block"><a href="{$category.url}" title="{$category.name}">{if $category.title}{$category.title|truncate:35:'...'}{else}{$category.name|truncate:35:'...'}{/if}</a></h3>
                    <p>{$category.subtitle}</p>
                    {/if}</div>
                    {if $display_show_more == 1}<a href="{$category.url}" title="{$category.name}" class="{if $display_text_hover == 2}btn-line{elseif $display_text_hover == 1}btn-line-under{/if}">{l s='Show more' d='Shop.Theme.Transformer'}</a>{/if}
