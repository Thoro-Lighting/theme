...
//<![CDATA[
    ...
    {/literal}
        ...
        stowlcarousel_array[{/literal}{$js_data.id_st_owl_carousel_group}{if isset($st_time)}{$st_time}{/if}{literal}] = {
            {/literal}
            ...
            {if !empty($js_data.pag_nav_text)}paginationLabels:[{foreach $js_data['slide'] as $i => $row}'{$row.title|escape}'{if $i < count($js_data['slide'])-1},{/if}{/foreach}],{/if}
