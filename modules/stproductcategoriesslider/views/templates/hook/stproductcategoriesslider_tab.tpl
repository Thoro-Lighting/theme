...
<div id="pc_slider_block_container_{$hook_hash}" class="pc_slider_block_container block {if $tabs_class}privat_class_{$tabs_class}{/if} {if $border_tabs == 0}one_border{else}four_border{/if} {if $additional_option >0}desktop_tab{/if} mobile_tabs_{$additional_option} mobile_title_align_{$additional_option_title} mobile_tabs_align_{$additional_option_tabs} width_tabs_{$additional_option_full}" data-tab="{foreach $product_categories as $p_c}{if ((isset($p_c.products) && is_array($p_c.products)) || $p_c.aw_display) && $p_c@first}category_products_container_{$p_c.id_st_product_categories_slider}{/if}{/foreach}">
...
{if $title_align < 3 && $header_align > 0 && $additional_option != 0}
                    {foreach $product_categories as $p_c}
                        {if (isset($p_c.products) && is_array($p_c.products)) || $p_c.aw_display}
                            <div id="category_products_container_{$p_c.id_st_product_categories_slider}-t" class="nav-header {if $p_c@first} active{/if} tabs_heading_{$header_align} head_{$title_align}_{$header_align} {if $p_c.padding_header == 1}yes{/if}">
                                <h3 class="title_block_inner pc_slider">{$p_c.title}</h3>
 ...
 <div class="title_block flex_container block">
                  {if $title_align == 3 && $header_align > 0 && $additional_option != 0}<div class="flex_child title_flex_left">
                    {foreach $product_categories as $p_c}
                        {if (isset($p_c.products) && is_array($p_c.products)) || $p_c.aw_display}
                            <div id="category_products_container_{$p_c.id_st_product_categories_slider}-t" class="nav-header {if $p_c@first} active{/if} tabs_heading_{$header_align} head_{$title_align}_{$header_align} {if $p_c.padding_header == 1}yes{/if}">
                                <h3 class="title_block_inner pc_slider">{$p_c.title}</h3>
                        ...
                        {/if}
                    {/foreach}</div>{/if}
                    <div class="flex_child title_flex_right swiper-container tabs_plus"><ul class="swiper-wrapper nav nav-tabs tab_lg flex_container {if $title_align==3 OR $title_align==2}flex_right{elseif $title_align == 4 OR $title_align==0}flex_left{else}flex_center{/if}">
        {*to do what if the first is hidden ? *}
        {foreach $product_categories as $p_c}
        {if (isset($p_c.products) && is_array($p_c.products)) || $p_c.aw_display}
        <li class="nav-item swiper-slide">
          <a class="nav-link{if $p_c@first} active{/if} {if $p_c.hide_mob == 1} hidden-md-down {elseif $p_c.hide_mob == 2} hidden-lg-up {/if} text-uppercase"  href="#category_products_container_{$p_c.id_st_product_categories_slider}" data-toggle="tab" aria-controls="category_products_container_{$p_c.id_st_product_categories_slider}">{$p_c.name}</a>
        </li>
        {/if}
        {/foreach}
    </ul>
    ...
    {if $title_align == 4 && $header_align > 0 && $additional_option != 0}<div class="flex_child title_flex_left">
                    {foreach $product_categories as $p_c}
                        {if (isset($p_c.products) && is_array($p_c.products)) || $p_c.aw_display}
                            <div id="category_products_container_{$p_c.id_st_product_categories_slider}-t" class="nav-header {if $p_c@first} active{/if} tabs_heading_{$header_align} head_{$title_align}_{$header_align} {if $p_c.padding_header == 1}yes{/if}">
                                <h3 class="title_block_inner pc_slider">{$p_c.title}</h3>
    ...
    <div class="tab-content">
        ...
        {if (isset($p_c.products) && is_array($p_c.products)) || $p_c.aw_display}
        <div role="tabpanel" class="block {$p_c.home_blog_add_class} products_container tab-pane {if $p_c@first} active st_open {/if} {if $p_c.hide_mob == 1} hidden-md-down {elseif $p_c.hide_mob == 2} hidden-lg-up {/if} {if $p_c.video_mpfour} video_bg_block {/if}" id="category_products_container_{$p_c.id_st_product_categories_slider}" 
        ...
        {if $p_c.video_mpfour} data-vide-bg="mp4: {$p_c.video_mpfour}{if $p_c.video_webm}, video_webm: {$p_c.video_webm}{/if}{if $p_c.video_ogg}, ogv: {$p_c.video_ogg}{/if}{if $p_c.video_poster}, poster: {$p_c.video_poster}{/if}" data-vide-options="video_loop: {if $p_c.video_loop}true{else}false{/if}, video_muted: {if $p_c.video_muted}true{else}false{/if}, position: 50% {(int)$p_c.video_v_offset}%" {/if}>
         {if $additional_option == 0}<div class="mobile_tab_title p_c_slider" data-id="#category_products_container_{$p_c.id_st_product_categories_slider}">
