...
<div id="stproductcomments_container_{$hook_hash}" class="stproductcomments_container {if $hide_mob == 2} hidden-lg-up-991 {elseif $hide_mob == 1} hidden-sm-down{/if} block {if !$column_fix && $has_background_img && $speed} st_parallax_block {/if} products_container{$column_fix} {if $column_slider && !isset($is_quarter)} column_block {/if} {if !$column_fix && $video_mpfour} video_bg_block {/if}" 
...
<section class="products_section">
    ...
    <div class="title_block flex_container title_align_{(int)$title_position} title_style_{(int)$sttheme.heading_style}">
        ...
        {if isset($view_more) && $view_more == 2}<div class="view_more_top"><a href="{url entity='module' name='stproductcomments' controller='list'}" class="btn btn-default" title="{l s='View more' d='Shop.Theme.Transformer'}">{l s='more' d='Shop.Theme.Transformer'}</a></div>{/if}
    ...
    {if isset($pcomments) && is_array($pcomments) && count($pcomments)}
        <div class="block_content {if !$column_slider && $content_width} width_{$content_width} {/if} static_bullets">
            ...
            <div class="swiper-container products_sldier_swiper {if $direction_nav>1} swiper-button-lr {if $direction_nav==6 || $direction_nav==7} swiper-navigation-circle {elseif $direction_nav==4 || $direction_nav==5} swiper-navigation-rectangle  {elseif $direction_nav==8 || $direction_nav==9} swiper-navigation-arrow {elseif $direction_nav==2 || $direction_nav==3} swiper-navigation-full {/if} {if $direction_nav==2 || $direction_nav==4 || $direction_nav==6|| $direction_nav==8} swiper-navigation_visible {/if}{/if}" {if $sttheme.is_rtl} dir="rtl" {/if}>
                <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide text-2">
                        {include file='module:stproductcomments/views/templates/hook/pcomments_item_slider.tpl'}
                    </div>
                    {/foreach}
                </div>
                {if $direction_nav>1}
            ...
            {include file="catalog/slider/script.tpl" block_name="#stproductcomments_container_{$hook_hash}"
