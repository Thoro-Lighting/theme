...
// use PrestaShop\PrestaShop\Adapter\ObjectPresenter;//Moved to the position below, for jian rong xing, bu zai zhe li sheng ming
// use PrestaShop\PrestaShop\Adapter\Presenter\Object\ObjectPresenter; //

class StDeliveryOptionsFinder
{
    private $context;
    ...
    public function getSelectedDeliveryOption()
    {
        ...
        return is_array($delivery_option) ? current($delivery_option) : $delivery_option;
    }
    ...
    {
        $delivery_option_list = $this->context->cart->getDeliveryOptionList(null, 1);//st flush cache, after login
                    ...
                    if (is_array($carriers)) {
                        foreach ($carriers as $carrier) {
                            ...
                            if ($this->isFreeShipping($this->context->cart, $carriers_list)) {
                                $carrier['price'] = Module::getInstanceByName('steasycheckout')->l('Free', 'StDeliveryOptionsFinder');
                            } else {
                                if ($include_taxes) {
                                    $carrier['price'] = $this->priceFormatter->format($carriers_list['total_price_with_tax']);
                                    if ($display_taxes_label) {
                                        $carrier['price'] = sprintf(Module::getInstanceByName('steasycheckout')->l('%s tax incl.', 'StDeliveryOptionsFinder'), $carrier['price']);
                                    }
                                } else {
                                    $carrier['price'] = $this->priceFormatter->format($carriers_list['total_price_without_tax']);
                                    if ($display_taxes_label) {
                                        $carrier['price'] = sprintf(Module::getInstanceByName('steasycheckout')->l('%s tax excl.', 'StDeliveryOptionsFinder'), $carrier['price']);
                                    }
                                }
                            }
