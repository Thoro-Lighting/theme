    ...
    if(typeof(instagram_block_array) ==='undefined')
        ...
        {literal}
            ...
            swiper: {
                autoplay:{
                    {/literal}
                    delay: {$in.s_speed|default:5000},
                    disableOnInteraction: {if $in.pause_on_hover}true{else}false{/if}
                    {literal}
                },
                {/literal}
                {if $in.direction_nav}
                    ...
                    navigation:{
                    {/literal}
                      nextEl: '#instagram_block_container_{$in.id_st_instagram} .swiper-button-next',
                      prevEl: '#instagram_block_container_{$in.id_st_instagram} .swiper-button-prev'
                    {literal}
                    },
                    {/literal}
                {/if}
                {if $in.control_nav}
                    ...
                    pagination: {
                    {/literal}
                      el: '#instagram_block_container_{$in.id_st_instagram} .swiper-pagination',
                      clickable: true,
                      {if $in.control_nav==2}
                        {literal}
                        ...
                        {/literal}
                        {/if}
                      type: {if $in.control_nav==2}'bullets'{elseif $in.control_nav==3}'progress'{else}'bullets'{/if} //A bug of swiper, this should be 'custom' when nav==2
                    {literal}
                    },
                    {/literal}
                {/if}
                ...
                slidesPerView: {$in.pro_per_xs|default:2},
                {if $in.move==1}slidesPerGroup: {$in.pro_per_xs|default:2},{/if}
                {if $sttheme.responsive && !$sttheme.version_switching}
                {literal}
                breakpoints: {
                    {/literal}
                    {if isset($homeverybottom) && $homeverybottom && $in.pro_per_fw}
                    {if $sttheme.responsive_max==2}1600{elseif $sttheme.responsive_max==1}1440{else}1200{/if}{literal}: {slidesPerView: {/literal}{$in.pro_per_fw|default:3}{if $in.move==1}, slidesPerGroup: {$in.pro_per_fw|default:3}{/if}{literal} },{/literal}
                    {/if}
                    {if $sttheme.responsive_max==2}{literal}1400: {slidesPerView: {/literal}{$in.pro_per_xxl|default:3}{if $in.move==1}, slidesPerGroup: {$in.pro_per_xxl|default:3}{/if}{literal} },{/literal}{/if}
                    {if $sttheme.responsive_max>=1}{literal}1200: {slidesPerView: {/literal}{$in.pro_per_xl|default:3}{if $in.move==1}, slidesPerGroup: {$in.pro_per_xl|default:3}{/if}{literal} },{/literal}{/if}
                    992: {literal}{slidesPerView: {/literal}{$in.pro_per_lg|default:2}{if $in.move==1}, slidesPerGroup: {$in.pro_per_lg|default:2}{/if}{literal} },{/literal}
                    768: {literal}{slidesPerView: {/literal}{$in.pro_per_md|default:4}{if $in.move==1}, slidesPerGroup: {$in.pro_per_md|default:4}{/if}{literal} },{/literal}
                    420: {literal}{slidesPerView: {/literal}{$in.pro_per_sm|default:3}{if $in.move==1}, slidesPerGroup: {$in.pro_per_sm|default:3}{/if}{literal} }
                },
                {/literal}
                {/if}
                speed: {$in.a_speed|default:400},
                lazy: false
            {literal}
            },
            {/literal}
