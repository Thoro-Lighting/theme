...
class StCountDown extends Module
{
    ...
    public function hookDisplayHeader($params)
    {
        ...
        /*
            iWeb - 2019-11-30
        */
        if ( $controller && !Configuration::get($this->_prefix_st.strtoupper('display_on_'.$controller)) )  return;
        ...
        if (!$this->isCached('module:stcountdown/views/templates/hook/header.tpl', $cache_id))
        {
            ...
            $custom_css .= '.countdown_wrap.all{height:'.($height).'px;}';
            
            if($padding = (int)Configuration::get($this->_prefix_st.'PADDING'))
            $custom_css .= '.countdown_wrap.all {padding-top:'.($padding).'px; padding-bottom:'.($padding).'px;}';
            ...
            if ($font_size = Configuration::get($this->_prefix_st.'FONT_SIZE'))
                ...
                $custom_css .= '.countdown_wrap.countdown_timer p {font-size:'.$font_size.'px;}';
